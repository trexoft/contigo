Vue.component('addwfst', {
  ...
  methods:{
      ...
      open:function(){
        this.onoff = true;
        this.setPage('tab2');
        GL.touch.on('#addwfst',function(event,direction){
          debugger;
      ...
      saveLayer:function(){
        ...
        if (this.layerName==""){
          ...
          GL.uyari("Servis Katman Adı Boş Olamaz");
            ...
            GL.uyari("Çalışma Alan Adını Giriniz");
        }else{
          ...
          $.get(url2,function(data, status){
            ...
            var information={namespace:that.namespace,url:that.url,typename:that.servicelayer,id:id,name:that.layerName,geotype:'wfst',layers:[id+"-point",id+"-line",id+"-polygon"]};
            ...
            GL.createLayerHistory3("createWFST",id,that.layerName,that.url,that.servicelayer,"wfst",that.versions.selected,style,that.namespace);  
        });
          ...
          GL.createLayerHistory3("createWFST",id,this.layerName,this.url,this.servicelayer,"wfst",this.versions.selected,style);
            ...
            '<div class="section full mt-1">'+
                '<div class="wide-block pt-2 pb-2">'+
                  ...
                  '<div class="form-group boxed" style="padding-left:5%; padding-right:5%;">'+
                        ...
                        '<div class="form-group boxed" style="padding-left:5%; padding-right:5%;">'+
                            '<div class="input-wrapper">'+
                                '<label class="label" for="urlwfst">URL</label>'+
                                '<input type="text" v-model="url" class="form-control" id="urlwfst" placeholder="URL adresini girin">'+
                                '<i class="clear-input">'+
                                    '<ion-icon name="close-circle"></ion-icon>'+
                                '</i>'+
                            '</div>'+
                        ...
                        '<div class="form-group boxed" style="padding-left:5%; padding-right:5%;">'+
                            '<div class="input-wrapper">'+
                                '<label class="label" for="namespace">Çalışma Alanı Adı</label>'+
                                '<input type="text" v-model="namespace" class="form-control" id="namespace" placeholder="Çalışma Alanının Adını Girin">'+
                                '<i class="clear-input">'+
                                    '<ion-icon name="close-circle"></ion-icon>'+
                                '</i>'+
                            '</div>'+
                        ...
                        '<div class="form-group boxed" style="padding-left:5%; padding-right:5%;">'+
                            '<div class="input-wrapper">'+
                                '<label class="label" for="columnwfst">Geometri Sütun Adı</label>'+
                                '<input type="text" v-model="geometryColumn" class="form-control" id="columnwfst" placeholder="Geometri sütun adını girin">'+
                                '<i class="clear-input">'+
                                    '<ion-icon name="close-circle"></ion-icon>'+
                                '</i>'+
                            '</div>'+
            ...
            '<div class="section full mt-1">'+
                '<div class="wide-block pt-2 pb-2">'+
                    ...
                    '<ul class="listview link-listview">'+
                        '<li v-for="(item,i) in layers" v-if="item.method==\'createWFST\'" style="padding:10px;">'+
                                '<div>{{item.name}}</div>'+
