Vue.component('addpbf', {
  ...
  methods:{
      ...
      open:function(){
        this.onoff = true;
        this.setPage('tab2');
        GL.touch.on('#addpbf',function(event,direction){
          debugger;
            ...
            '<div class="section full mt-1">'+
                '<div class="wide-block pt-2 pb-2">'+
                    ...
                    '<ul class="listview link-listview">'+
                        '<li v-for="(item,i) in layers" v-if="item.method==\'createPBF\'" style="padding:10px;">'+
                                '<div>{{item.name}}</div>'+
                                '<div class="custom-control custom-switch">'+
                                '<button type="button" @click="deleteLayer(item,i)" class="btn btn-text-primary">Sil</button>'+
